...
\frame{
  \frametitle{About me (for some context!)}
  \vfill
  Some recent research interests:
  \begin{itemize}
  \item Compatible finite element methods for atmosphere and ocean
    modelling
  \item Data assimilation for stochastic PDEs
  \item Automated code generation with Firedrake
  \item Structure preserving numerics esp. energy
    conserving/consistent schemes
  \item Parallel-in-Time numerics
  \item Scalable solvers for the indefinite Helmholtz equation
  \item Numerical methods for optimal transport and application to
...
\frame{
  ...
  \vfill
  \centerline{\includegraphics[width=6cm]{complexity}}
  \begin{block}{}
